...
`).map(s=>s.trim()).filter(Boolean),i={...t,professional:e};k(i,"skills.json"),y("export","skills","Exported skills.json"),c("skills.json exported")}),document.getElementById("publishBtn").addEventListener("click",()=>{const e=document.getElementById("profSkills").value.split(`
`).map(s=>s.trim()).filter(Boolean),i={...t,professional:e};T("skills",()=>B("public/data/skills.json",JSON.stringify(i,null,2),"Update skills from PortfolioOS"))});function d(e){const i=e<0,s=i?{name:"",icon:"fa-solid fa-code",category:"frontend"}:{...t.programming[e]},m=`
    ...
    `;I(i?"Add Programming Skill":"Edit Programming Skill",m,w(i)),document.getElementById("modalCancelBtn").addEventListener("click",g),document.getElementById("modalSaveBtn").addEventListener("click",()=>{const u=document.getElementById("f_name").value.trim();if(!u){c("Skill name is required","error");return}const p={name:u,icon:v("f_icon"),category:document.getElementById("f_category").value};i?t.programming.push(p):t.programming[e]=p,l(),g(),y("edit","skills",i?`Added skill "${p.name}"`:`Edited skill "${p.name}"`),c(i?"Skill added":"Skill saved")})}}function Z(n){let t=[];n.innerHTML=`
